...
<div class="row-fluid">
		...
		action="${pageContext.request.contextPath }/magazine!editYear.action">
							<input type="hidden" name="year.id" value="<s:property value="#request.edityear.id" />" />
							<input type="hidden" name="year.statu" value="<s:property value="#request.edityear.statu" />" />
							
							<label class="control-label" for="input01">修改分类编号</label><br />
							<input type="text" class="x-large" id="yearname" name="year.yearname" value="<s:property value="#request.edityear.yearname" />" />
							<p>格式为阿拉伯四位数字年份，如 2013</p>
									
							<label class="control-label" for="input01">修改窗帘一级分类名称</label><br />
							<input type="text" class="x-large" id="yeartitle" name="year.title" value="<s:property value="#request.edityear.title" />" />
							<p>输入窗帘的一级分类，如 田园风格 </p>
							<div style="color:red;font-size:1.5ex;">${editYearError }</div>
							<input name="submit" value="修改" class="btn" id="addyear" type="submit" />
						</form>
								...
								<tbody>
									...
									<s:iterator value="#request.yearlist" var="info">
        								<tr>
            								...
            								<td>
            									<!-- 状态为被删除的，还原 -->
              								<a href="${pageContext.request.contextPath }/magazine!updateYear.action?yid=<s:property value="id" />"><i class="icon-ok"></i></a>
            								</td>
            								</s:if>
